import os
import csv
import random
from tqdm import tqdm
from collections import defaultdict

import albumentations
import albumentations.pytorch

import torch
from torch.utils.data import Dataset
from vae import VQGanVAE
from PIL import Image

random.seed(42)

class UnifiedCXRDataset(Dataset):

    def __init__(self,
                 metadata_file,
                 img_root_dir,
                 text_root_dir,
                 vqgan_model_path,
                 vqgan_config_path,
                 codebook_indices_path,  # ← 사용 안 함
                 vqgan,
                 max_img_num,
                 max_text_len,
                 tokenizer,
                 target_count,
                 target_view,
                 under_sample="fixed"
                 ):
        super().__init__()

        assert max_img_num <= target_count, f'max_img_num({max_img_num}) should be less than target_count({target_count}).'

        self.under_sample = under_sample.split('_')[0]
        self.select_studies = under_sample.split('_')[1]
        self.training_mode = under_sample.split('_')[-1]

        self.dict_by_studyid = defaultdict(list)

        with open(metadata_file, 'r') as f:
            rdr = csv.reader(f)
            for line in tqdm(rdr):
                dicom_id, subject_id, study_id, ViewPosition, count = line
                if self.select_studies == 'each':
                    if int(count) == int(target_count) and ViewPosition in target_view:
                        self.dict_by_studyid[study_id].append(line)
                elif self.select_studies == 'all':
                    if int(count) <= int(target_count) and ViewPosition in target_view:
                        self.dict_by_studyid[study_id].append(line)

        if self.select_studies == 'all':
            self.dict_by_studyid = {k: v for k, v in self.dict_by_studyid.items()
                                    if len(v) == int(v[0][-1])}
        elif self.select_studies == 'each':
            self.dict_by_studyid = {k: v for k, v in self.dict_by_studyid.items()
                                    if len(v) == target_count}

        self.key_list = list(self.dict_by_studyid.keys())

        self.img_root_dir = img_root_dir
        self.text_root_dir = text_root_dir

        self.vae = VQGanVAE(vqgan_model_path, vqgan_config_path)

        if vqgan == 512:
            self.img_fmap_size = 32
            self.img_reso = 512
            self.img_len = 1024 + 2
            self.img_vocab_size = self.vae.num_tokens
        else:
            raise NotImplementedError

        self.max_img_num = max_img_num
        self.target_count = target_count
        self.max_text_len = max_text_len

        self.tokenizer = tokenizer
        self.text_vocab_size = self.tokenizer.get_vocab_size()

        self.rescaler = albumentations.SmallestMaxSize(max_size=self.img_reso)
        self.cropper = albumentations.CenterCrop(height=self.img_reso, width=self.img_reso)
        self.totensor = albumentations.pytorch.transforms.ToTensorV2()
        self.preprocessor = albumentations.Compose([
            self.rescaler,
            self.cropper,
            self.totensor,
        ])

        self.slots = []
        self.modes = ['txt']
        for i in range(self.max_img_num):
            y = [self.img_vocab_size + i] * (self.img_len)
            self.slots.extend(y)
            self.modes.append(f'img{i + 1}')

    def __len__(self):
        return len(self.key_list)

    def __getitem__(self, idx):
        study_id = self.key_list[idx]
        imgs_meta = self._select_images(study_id)

        image_output = []
        view_position = []
        img_paths = ''

        for i, line in enumerate(imgs_meta):
            dicom_id, subject_id, studyid, ViewPosition = line[:4]
            img_path = os.path.join(self.img_root_dir, 'p' + subject_id[:2], 'p' + subject_id, 's' + studyid, dicom_id + '.jpg')
            img_paths += img_path + '|'
            view_position.append(ViewPosition)

            image = Image.open(img_path).convert("RGB")
            image = self.preprocessor(image=np.array(image))['image'].unsqueeze(0)  # [1, C, H, W]
            indices = self.vae.get_codebook_indices(image)[0]  # [1024]

            # 시퀀스 앞뒤에 토큰 추가
            if ViewPosition == 'AP':
                indices = torch.cat([torch.tensor([1025]), indices, torch.tensor([1026])])
            elif ViewPosition == 'PA':
                indices = torch.cat([torch.tensor([1027]), indices, torch.tensor([1028])])
            elif ViewPosition in ['LATERAL', 'LL']:
                indices = torch.cat([torch.tensor([1029]), indices, torch.tensor([1030])])
            else:
                raise ValueError(f"Unexpected view: {ViewPosition}")

            image_output.append(indices)

        # padding (for max_img_num)
        if len(imgs_meta) < self.max_img_num:
            for i in range(self.max_img_num - len(imgs_meta)):
                pad = torch.tensor([1024] * self.img_len)
                image_output.append(pad)
                img_paths += 'PAD|'
                view_position.append('PAD')

        random.shuffle(self.modes)

        # 텍스트 로딩
        text_path = os.path.join(self.text_root_dir, 's' + study_id + '.txt')
        with open(text_path, 'r') as f:
            text = f.read()
        src = text.replace('  ', ' ').replace('  ', ' ').lower()
        text_ids = self.tokenizer.encode(src).ids
        text_output = torch.tensor(text_ids[:self.max_text_len])

        outputs = {
            'txt': text_output,
            'modes': self.modes,
            'study_id': study_id,
            'img_paths': img_paths,
            'view_position': view_position
        }
        for i in range(self.max_img_num):
            outputs[f'img{i+1}'] = image_output[i]

        return outputs

    def _select_images(self, study_id):
        if self.max_img_num == self.target_count:
            return self.dict_by_studyid[study_id]
        if self.under_sample == 'fixed':
            return self.dict_by_studyid[study_id][:self.max_img_num]
        elif self.under_sample == 'random':
            return random.sample(self.dict_by_studyid[study_id], self.max_img_num)